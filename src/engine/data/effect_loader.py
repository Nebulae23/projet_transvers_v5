...
def load_effect_data(data_path: Path):
    """Charge les données des effets visuels depuis le fichier JSON."""
    file_path = data_path / "effects.json"
    if not file_path.is_file():
        # Gérer l'erreur si le fichier n'existe pas
        print(f"Erreur : Fichier de données des effets introuvable à {file_path}")
        return {}
...
if __name__ == '__main__':
    ...
    effects = load_effect_data(data_dir)
    print("Données des effets chargées :")
    print(effects)