...
def load_ability_data(data_path: Path):
    """Charge les données des capacités depuis le fichier JSON."""
    file_path = data_path / "abilities.json"
    if not file_path.is_file():
        # Gérer l'erreur si le fichier n'existe pas
        print(f"Erreur : Fichier de données des capacités introuvable à {file_path}")
        return {}
...
if __name__ == '__main__':
    ...
    abilities = load_ability_data(data_dir)
    print("Données des capacités chargées :")
    print(abilities)